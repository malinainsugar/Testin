...
class report:
    def __init__(self, nameProfession):
        ...
        self.statisticsYear.append(['Год', 'Средняя зарплата', f'Средняя зарплата - {self.nameProfession}', 'Количество вакансий', f'Количество вакансий - {nameProfession}'])
    ...
    def generate_pdf(self, dataList : list):
      ...
      pdf_template = template.render({'nameProfessonal': self.nameProfession, 'image_file' : 'graph.png', 'dataList' : dataList})
...
def dataProcessing():
  ...
  statement = report(parameters.nameProfession)
  statement.generate_excel(dataList)
  statement.generate_image(dataList)
  statement.generate_pdf(dataList)
